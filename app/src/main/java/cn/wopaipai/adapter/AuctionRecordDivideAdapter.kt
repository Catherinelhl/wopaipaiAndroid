...
+--------------+---------------------------------
+ description  +   适配器：分成记录
+--------------+---------------------------------
+ version      +  
+--------------+---------------------------------
*/

class AuctionRecordDivideAdapter(
    private val context: Context
) : RecyclerView.Adapter<AuctionRecordDivideAdapter.viewHolder>() {
    private val TAG = AuctionRecordDivideAdapter::class.java.simpleName
    private var auctionRecordDivideBeans: ArrayList<AuctionRecordDivideBean> = arrayListOf()
    @SuppressLint("SetTextI18n")
    override fun onBindViewHolder(viewHolder: viewHolder, position: Int) {
        auctionRecordDivideBeans?.let {
            ...
            viewHolder.tvCurrentPrice.text = auctionRecordOfferBean.dateTime
            viewHolder.tvIncrementPrice.text =
                "${auctionRecordOfferBean.amount}\t\t${MessageConstants.AUCTION_TYPE}"
        }
    }
    ...
    override fun getItemCount(): Int =
        if (ListTool.isEmpty(auctionRecordDivideBeans)) 0 else auctionRecordDivideBeans!!.size

    /**
     * 重新添加数据
     */
    fun addList(auctionRecordDivideBeans: ArrayList<AuctionRecordDivideBean>) {
        this.auctionRecordDivideBeans?.let {
            it.clear()
            it.addAll(auctionRecordDivideBeans)
