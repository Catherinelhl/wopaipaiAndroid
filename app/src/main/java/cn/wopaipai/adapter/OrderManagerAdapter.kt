...
class OrderManagerAdapter(
    ...
    private var orderManagerBeans: ArrayList<OrderManagerBean>
) : RecyclerView.Adapter<OrderManagerAdapter.viewHolder>() {
    ...
    override fun onBindViewHolder(viewHolder: viewHolder, position: Int) {
        orderManagerBeans?.let {
            ...
            viewHolder.tvName.text = acquisitionBean.productName
            viewHolder.tvInfo.text =context!!.resources.getString(R.string.auction_price)+"\t"+
                    acquisitionBean.price+"\t"+MessageConstants.AUCTION_TYPE+"\t"+context.resources.getString(R.string.get_in)
            viewHolder.tvTime.text = acquisitionBean.obtainTime
            GlideImgManager.displayImage(context,acquisitionBean.imageUrl,viewHolder.ivCover)
    ...
    override fun getItemCount(): Int =
        if (ListTool.isEmpty(orderManagerBeans)) 0 else orderManagerBeans!!.size
